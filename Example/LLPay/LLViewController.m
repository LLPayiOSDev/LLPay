...
LLPayType payType = LLPayTypeVerify;
...
- (void)lianliansign {
    ...
    /*  签名操作， 请注意⚠️
     ...
     但是在真实APP中， 请务必⚠️将privateKey和加签过程放在商户服务端⚠️
     以防止商户私密数据泄露，造成不必要的资金损失，及面临各种安全风险" */
    self.signUtil.signKeyArray = @[@"acct_name",@"card_no",@"id_no",@"oid_partner",@"risk_item",@"sign_type",@"user_id",@"repayment_plan",@"repayment_no",@"sms_param"];
...
- (NSDictionary *)createLLOrder {
    ...
    //======================= 分期参数 =======================//
    //    orderParams[@"repayment_plan"] = [LLPayUtil jsonStringOfObj:@{@"repaymentPlan":@[@{@"date" : @"2017-05-03", @"amount" : @"0.01"},@{@"date" : @"2017-06-03", @"amount" : @"0.02"}]}];
    //    orderParams[@"repayment_no"] = @"";
    //    orderParams[@"sms_param"] = [LLPayUtil jsonStringOfObj:@{@"contract_type" : @"短信显示的商户名",@"contact_way" : @"400-018-8888"}];
...
#pragma mark - UserInterface Functions

- (void)llDemoUI {
    [self llBtnWithName:@"SDK版本" selector:@selector(sdkVersion) index:0];
    [self llBtnWithName:@"支付" selector:@selector(lianlianpay) index:1];
    [self llBtnWithName:@"签约" selector:@selector(lianliansign) index:2];
    UILabel *label = [[UILabel alloc] init];
    label.frame = CGRectMake(margin, height - margin - 100 - 110, width, 100);
    label.layer.cornerRadius = corner;
    label.backgroundColor = [UIColor whiteColor];
    label.layer.borderColor = LLColor.CGColor;
    label.layer.borderWidth = 1;
    label.text = @"\t\tDemo中为了方便演示， 所以将加签过程以及密钥放在本地\n\t\t但是在真实APP中， 请务必\n\t\t⚠️将privateKey和加签过程放在商户服务端⚠️\n\t\t以防止商户私密数据泄露，造成不必要的资金损失，及面临各种安全风险";
...
- (void)llBtnWithName: (NSString *)name selector:(SEL)selector index: (NSUInteger)index {
    UIButton *payButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [payButton setTitle:name forState:UIControlStateNormal];
    [payButton setBackgroundColor:LLColor];
    payButton.frame = CGRectMake(margin, margin * 2 + (margin + itemHeight) * index, width, itemHeight);
    payButton.layer.cornerRadius = corner;
    [payButton addTarget:self action:selector forControlEvents:UIControlEventTouchUpInside];
